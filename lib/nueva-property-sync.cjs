...
function normalizePropertyPayload(input, { infer = false } = {}) {
  ...
  return compactPayload({
    ...
    public_name: cleanString(input.public_name),
...
// The CRM wants a date; the site stores a quarter. This derives one from
// the website's own data and nothing else, so what the CRM holds is always
// what the site shows.
//
// discovery.completionSort ("2028-2") is the value the move-in filter runs
// on, and hero.delivery ("Q2 2028") is the text on the card -- either can
// answer the question, so the second covers a project that has not been
// given an explicit sort. There used to be a third copy, crm.deliveryDate,
// kept by hand: it was empty on eight of fourteen projects and stale on
// another, still saying 2027-10-01 after that delivery moved to Q2 2028.
// A fact stored twice is a fact that disagrees with itself eventually.
//
// Mapped to the first day of the quarter, the convention the field already
// used.
function deliveryDateFromSite(project) {
  const explicit = /^(\d{4})-([1-4])$/.exec(String(project.discovery?.completionSort || '').trim());
  const quarter = explicit || (() => {
    const text = String(project.hero?.delivery || '');
    const q = text.match(/Q([1-4])\s*(20\d{2})/i);
    if (q) return [null, q[2], q[1]];
    const year = text.match(/20\d{2}/);
    return year ? [null, year[0], '4'] : null;
  })();
...
function projectToPropertyPayload(project, { siteUrl = DEFAULT_SITE_URL } = {}) {
  ...
  return normalizePropertyPayload({
    // For anonymized public listings, crm.realName/realDeveloper hold the
    // actual project name and developer -- never shown on the site, but
    // required so the CRM's own records stay accurate. Falls back to the
    // public name/developer when a project isn't anonymized.
    name: crm.realName || project.name,
    // The name the website shows. Sent only when it differs from the real
    // one, so a project that is not anonymised does not store the same
    // string twice. Without this the CRM held "ZEW" and nobody could find
    // the record by "Elviria Hills Residences" -- the name on the site, and
    // the one a buyer says on the phone.
    public_name: crm.realName && crm.realName !== project.name ? project.name : undefined,
    developer: crm.realDeveloper || crm.developer,
    ...
    delivery_date: deliveryDateFromSite(project),
