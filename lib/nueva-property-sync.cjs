...
// The CRM wants a date; the site stores a quarter. discovery.completionSort
// ("2028-2") is the value the site's own move-in filter runs on, so it is
// the one source that is guaranteed current -- crm.deliveryDate was a
// hand-kept second copy that was empty on eight projects and stale on
// another, still saying 2027-10-01 after the delivery moved to Q2 2028.
// Mapped to the first day of the quarter, matching the values already in
// the field.
function deliveryDateFromCompletion(completionSort) {
  const match = /^(\d{4})-([1-4])$/.exec(String(completionSort || '').trim());
...
function projectToPropertyPayload(project, { siteUrl = DEFAULT_SITE_URL } = {}) {
  ...
  return normalizePropertyPayload({
    ...
    delivery_date: deliveryDateFromCompletion(project.discovery?.completionSort) || crm.deliveryDate,
