...
for slug in sorted(os.listdir(ROOT)):
    ...
    if not os.path.isdir(project_dir):
        continue
    # The floorplan thumbnails are listed too. They were generated by hand for
    # every earlier project, which meant a new project shipped its plans with
    # no WebP at all until someone noticed. They take the two narrow widths
    # only -- a full-width copy of a thumbnail is never requested. The spin
    # posters are the same shape of thing: the still shown before a visitor
    # asks to turn the plan, never displayed wider than a card.
    for folder in (project_dir, os.path.join(project_dir, "media"),
                   os.path.join(project_dir, "floorplans"),
                   os.path.join(project_dir, "spins")):
...
for src in sources:
    ...
    narrow = any(os.sep + folder + os.sep in src for folder in ("floorplans", "spins"))
    widths = WIDTHS[1:] if narrow else WIDTHS
