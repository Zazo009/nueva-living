...
let hiddenRuleChecked = 0;

// An author rule for a bare element or a class beats the user agent's
// `[hidden] { display: none }` whatever its specificity, so the attribute
// stops meaning anything the moment a stylesheet says `img { display: block }`
// or `.card-lightbox { display: flex }`. Both are real rules here, and the bug
// they cause is silent: the element is announced as hidden and drawn anyway.
// Four features had each patched it locally before the layout spins stacked
// twenty-one frames on one square. Every stylesheet that can be the whole
// stylesheet for a page now has to carry the rule once, with enough weight.
{
  const sheets = ['assets/liora/liora-pages.css', 'assets/liora/nueva-system.css'];
  // Bare `[hidden]`, not `.something[hidden]`: a rule scoped to one class is
  // the per-feature patch this guard exists to stop.
  const wanted = /(?:^|[},;])\s*\[hidden\]\s*\{[^}]*display:\s*none\s*!important/;
  for (const sheet of sheets) {
    ...
    const css = fs.readFileSync(file, 'utf8').replace(/\/\*[\s\S]*?\*\//g, '');
    if (!wanted.test(css)) {
      fail(sheet, 'carries no `[hidden] { display: none !important }` rule, so any element it '
        + 'gives a display value to goes on showing when JavaScript sets the hidden attribute.');
    }
  }
}
...
let metricLabelChecked = 0;

// One English metric label has one word per language.
//
// The overview grid's row labels are written per project, so the same
// English label drifted into synonyms: "Collection" was Ensemble, Programme
// and Collection in French and Kolekcja, Inwestycja and Zespol in Polish,
// and "Terraces" was simply left in English on two projects while seven
// others translated it. Each page read fine on its own, which is why none of
// this ever surfaced -- you only see it by putting the projects side by side,
// which is exactly what the developments grid does.
//
// The rule is narrow on purpose: it compares a label only against the same
// English label on other projects, so a genuine loanword is fine as long as
// every project uses it. It does not police the values beside the labels,
// which are prose and legitimately differ.
{
  const dir = path.join(root, 'content', 'liora-projects');
  const seen = new Map();
  if (fs.existsSync(dir)) {
    for (const slug of fs.readdirSync(dir)) {
      ...
      if (!fs.existsSync(file)) continue;
      const project = JSON.parse(fs.readFileSync(file, 'utf8'));
      const english = project.overview?.metrics;
      if (!Array.isArray(english)) continue;
      for (const locale of ATTR_LOCALES) {
        const rows = project.i18n?.[locale]?.overview?.metrics;
        if (!Array.isArray(rows) || rows.length !== english.length) continue;
        for (const [index, row] of english.entries()) {
          const label = row?.[0];
          const translated = rows[index]?.[0];
          if (typeof label !== 'string' || typeof translated !== 'string') continue;
          metricLabelChecked += 1;
          const key = `${locale}\u0000${label}`;
          if (!seen.has(key)) seen.set(key, new Map());
          if (!seen.get(key).has(translated)) seen.get(key).set(translated, slug);
        }
      }
    }
  }
  ...
  for (const [key, forms] of seen) {
    ...
    const [locale, label] = key.split('\u0000');
    const spelled = [...forms].map(([form, slug]) => `${JSON.stringify(form)} (${slug})`);
    offenders.push(`${locale} writes "${label}" as ${spelled.join(' and ')}`);
  }
  if (offenders.length) {
    fail('content/liora-projects', `${offenders.length} metric label(s) have more than one form in the same `
      + `language, so the same row reads differently from one project to the next: `
      + `${offenders.slice(0, 3).join('; ')}.`);
  }
}

let indexFactsChecked = 0;

// The developments index counts what it actually lists.
//
// Its title resolved {count} from the live project list; the description
// beside it was written out longhand and never did. It advertised "30
// developments" and "guide prices from EUR 305,000" to every search result
// while the title said 47 and the cheapest project had fallen to EUR 274,000,
// and nothing failed, because both were well-formed sentences.
{
  const file = path.join(dist, 'developments.html');
  const dir = path.join(root, 'content', 'liora-projects');
  if (fs.existsSync(file) && fs.existsSync(dir)) {
    const projects = fs.readdirSync(dir)
      .map((slug) => path.join(dir, slug, 'project.json'))
      .filter((f) => fs.existsSync(f))
    ...
    for (const [label, text] of [['title', title], ['description', description]]) {
      const counts = [...text.matchAll(/(?<![\d,€])(\d{1,3})(?![\d,])/g)].map((m) => Number(m[1]));
      if (!counts.length) continue;
      indexFactsChecked += 1;
      if (!counts.includes(projects.length)) {
        fail('dist/developments.html', `the ${label} says ${JSON.stringify(text.trim())} but the site `
          + `publishes ${projects.length} projects. The number belongs to the project list, not to a `
          + `sentence someone has to remember to update.`);
      }
    }
    const money = [...description.matchAll(/€([\d,]+)/g)].map((m) => Number(m[1].replace(/,/g, '')));
    if (money.length && prices.length) {
      indexFactsChecked += 1;
      const min = Math.min(...prices);
      if (!money.includes(min)) {
        fail('dist/developments.html', `the description quotes ${money.map((v) => `€${v.toLocaleString('en-US')}`).join(', ')} `
          + `but the cheapest published project is €${min.toLocaleString('en-US')}.`);
      }
    }
  }
}

let scriptMixChecked = 0;

// A translation uses its own alphabet.
//
// A Cyrillic "\u0430" is indistinguishable from a Latin "a" on screen, so one
// that lands in a Polish sentence looks perfect and is wrong everywhere it
// matters: search does not match it, the word breaks in fonts without
// Cyrillic coverage, and a copy-paste into a contract carries it along. Two
// arrived in this project the day it was written, from a keyboard switch
// mid-sentence, and nothing anywhere else would have noticed.
//
// Each locale is checked only against the alphabets it never uses, so Spanish
// accents, Polish diacritics and the Arabic and Russian overlays themselves
// are all untouched.
{
  const CYRILLIC = /[\u0400-\u04FF]/;
  const ARABIC = /[\u0600-\u06FF\u0750-\u077F]/;
  const FORBIDDEN = {
    es: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    fr: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    de: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    nl: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    pl: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    no: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    sv: [['Cyrillic', CYRILLIC], ['Arabic', ARABIC]],
    ru: [['Arabic', ARABIC]],
  ...
  if (fs.existsSync(dir)) {
    for (const slug of fs.readdirSync(dir)) {
      ...
      if (!fs.existsSync(file)) continue;
      const project = JSON.parse(fs.readFileSync(file, 'utf8'));
      for (const [locale, rules] of Object.entries(FORBIDDEN)) {
        const overlay = project.i18n?.[locale];
        if (!overlay) continue;
        walk(overlay, (text) => {
          scriptMixChecked += 1;
          for (const [name, re] of rules) {
            if (re.test(text)) {
              offenders.push(`${slug} [${locale}] has ${name} letters in ${JSON.stringify(text.slice(0, 60))}`);
              break;
            }
          }
        });
  ...
  if (offenders.length) {
    fail('content/liora-projects', `${offenders.length} translated string(s) mix in letters from another `
      + `alphabet, which look identical on screen but break search and font coverage: `
      + `${offenders.slice(0, 3).join('; ')}.`);
  }
}
...
let overlayStructuralChecked = 0;

// A locale overlay only ever restates the prose of an item; the structural
// fields around it -- an image src, a unit's floorplan, a timeline point's
// milestone flag -- are the same in every language and are not repeated.
// localizeProject used to replace an overlay array wholesale, so those fields
// simply vanished on the nine locale builds: every translated property page
// shipped with no floorplans, no gallery images and an empty payment
// schedule, and nothing failed because an absent field renders as nothing.
// The merge is element-wise now, and this is the proof: whatever English
// holds at one of these paths, the localized project has to hold too.
{
  const dir = path.join(root, 'content', 'liora-projects');
  const STRUCTURAL = new Set(['src', 'floorplan', 'href', 'width', 'height', 'milestone', 'icon']);
  const collect = (value, prefix, into) => {
    if (Array.isArray(value)) {
      value.forEach((item, index) => collect(item, `${prefix}[${index}]`, into));
    } else if (value && typeof value === 'object') {
      for (const [key, item] of Object.entries(value)) {
        if (key === 'i18n') continue;
        if (STRUCTURAL.has(key)) into.set(`${prefix}.${key}`, JSON.stringify(item));
        else collect(item, `${prefix}.${key}`, into);
      }
    }
    return into;
  };
  if (fs.existsSync(dir)) {
    ...
    for (const slug of fs.readdirSync(dir)) {
      ...
      if (!fs.existsSync(file)) continue;
      const project = JSON.parse(fs.readFileSync(file, 'utf8'));
      const english = collect(project, '', new Map());
      for (const locale of Object.keys(project.i18n || {})) {
        const localized = collect(localizeProject(project, locale), '', new Map());
        for (const [key, value] of english) {
          overlayStructuralChecked += 1;
          if (localized.get(key) !== value) {
            offenders.push(`${slug} [${locale}]${key}: ${localized.get(key) ?? 'missing'} against ${value}`);
          }
        }
      }
    }
    if (offenders.length) {
      fail('content/liora-projects', `${offenders.length} structural field(s) are lost when a project is `
        + `localized: ${offenders.slice(0, 4).join('; ')}.`);
    }
  }
}

let unitRatioChecked = 0;

// Casares Costa shipped a card reading "25 of 25 left". Nobody had counted the
// scheme: crm.totalUnits had been set from the length of the project's own
// availability table, so the ratio was the same number twice and the card
// asserted that not one home had sold. A count we read off a release list is
// evidence about that list, not about the whole development, so a card only
// prints a ratio when the total is genuinely larger than what is left. Both
// numbers are read back out of the rendered card here, in every language,
// because the English source is only one of ten places this renders.
{
  const RATIO = /<span class="dev-(?:badge-units|fact-value)">([^<]*\d[^<]*)<\/span>/g;
  const offenders = [];
  const pages = [];
  for (const entry of fs.readdirSync(dist, { withFileTypes: true })) {
    if (entry.isFile() && entry.name.endsWith('.html')) pages.push(entry.name);
    else if (entry.isDirectory() && /^[a-z]{2}$/.test(entry.name)) {
      for (const name of fs.readdirSync(path.join(dist, entry.name))) {
        if (name.endsWith('.html')) pages.push(path.join(entry.name, name));
      }
    }
  }
  for (const file of pages) {
    const html = fs.readFileSync(path.join(dist, file), 'utf8');
    for (const [, text] of html.matchAll(RATIO)) {
      const numbers = (text.match(/\d+/g) || []).map(Number);
      if (numbers.length !== 2) continue;
      unitRatioChecked += 1;
      if (numbers[0] === numbers[1] && numbers[0] > 1) {
        offenders.push(`${file}: "${text.trim()}"`);
      }
    }
  }
  if (offenders.length) {
    fail('cards', `${offenders.length} card(s) claim every home in the scheme is still `
      + `available: ${[...new Set(offenders)].slice(0, 4).join('; ')}.`);
  }
}

let dropdownDecorationChecked = 0;

// The area dropdown's options are anchors inside `.nav-links`, so every
// decoration written for a top-level nav item lands on them too. The hover
// underline is pinned to `left: 0; right: 0`, which on a full-width dropdown
// option ran a gold line across the whole panel and below its padding box --
// a highlight visibly escaping the menu. The cancel has to carry an id for
// the homepage's own `#nav .nav-links a:hover::after`, and has to name both
// nav shells, because `#nav` is the homepage and `.site-nav` is every other
// page type. Checked here rather than trusted, because the rule is invisible
// until someone hovers a menu.
{
  const sheet = 'assets/liora/nueva-nav-interactions.css';
  const file = path.join(root, sheet);
  if (fs.existsSync(file)) {
    const css = fs.readFileSync(file, 'utf8');
    // The block that cancels them, whatever else it lists.
    const block = css.match(/([^{}]*\.nav-dropdown-option::(?:before|after)[^{}]*)\{([^}]*)\}/);
    const selectors = block ? block[1] : '';
    const body = block ? block[2] : '';
    const missing = [];
    if (!block) missing.push('no rule cancels the pseudo-elements at all');
    else {
      if (!/content\s*:\s*none/.test(body)) missing.push('the rule does not set content: none');
      for (const shell of ['#nav', '.site-nav']) {
        for (const pseudo of ['::before', '::after']) {
          const wanted = `${shell} .nav-links .nav-dropdown-option${pseudo}`;
          dropdownDecorationChecked += 1;
          if (!selectors.includes(wanted)) missing.push(`no "${wanted}"`);
        }
      }
    }
    if (missing.length) {
      fail(sheet, `the nav dropdown would inherit the top-level hover underline: ${missing.join('; ')}.`);
    }
  }
}
