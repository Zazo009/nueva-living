...
function areaDetailPage(area) {
  ...
  const spotlightSection = area.spotlight ? `
    <section class="section quiet-band area-spotlight"><div class="section-inner"><div class="section-head"><span class="label">Local Knowledge</span><div class="rule"></div><h2 class="section-title">${area.spotlight.headlineHtml}</h2><p class="body-copy">${esc(area.spotlight.intro)}</p></div><div class="cards">${area.spotlight.items.map(([title, copy], index) => `<article class="card"><div class="card-number">${String(index + 1).padStart(2, '0')}</div><h3>${esc(title)}</h3><p>${esc(copy)}</p></article>`).join('')}</div></div></section>` : '';
  ...
  return {
    ...
    <section class="section area-market"><div class="section-inner area-market-layout"><div><span class="label">Price Context</span><div class="rule"></div><h2 class="section-title">Current asking-price <em>reference</em></h2><p class="body-copy">Use the area average for orientation only. New-build pricing depends heavily on the exact location, specification and views.</p></div><div class="area-price-panel">${priceItems}<p>${esc(area.priceNote)}</p><div class="area-price-sources">${areaPriceSources(area)}</div></div></div></section>
